...
<style>
    ...
    /* Стили для кастомных элементов внутри карточки */
    .custom-input {{
        flex: 1;
        min-width: 200px;
        padding: 14px 20px;
    ...
    /* Скрываем стандартный input Streamlit */
    .stTextInput {{
...
# ----------------------------------------------------------------------------
# 4. КАРТОЧКА ПОИСКА (полностью кастомная)
# ----------------------------------------------------------------------------
...
if 'inn_value' not in st.session_state:
    st.session_state.inn_value = ""

# Кастомная форма через HTML + JavaScript
st.markdown("""
<div class="search-card">
    <label for="innInput">ИНН компании</label>
    <div class="search-row">
        <input type="text" id="innInput" class="custom-input" placeholder="Введите 10 или 12 цифр" maxlength="12" value="">
        <button id="checkBtn" class="custom-btn">Проверить</button>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Функция для отправки значения в Streamlit
        function sendValue() {
            const value = input.value.trim();
            if (value) {
                // Отправляем через Streamlit.setComponentValue
        ...
        button.addEventListener('click', function(e) {
            e.preventDefault();
            sendValue();
            // Также эмулируем нажатие на скрытую кнопку Streamlit
...
# Скрытые Streamlit виджеты для логики
col_hidden_1, col_hidden_2 = st.columns([3, 1])
with col_hidden_1:
    inn = st.text_input(
        "",
        ...
        key="inn_input_hidden"
    )
with col_hidden_2:
    check_btn = st.button("Проверить", key="check_btn_hidden")

# Синхронизация значения между кастомным input и скрытым
if inn != st.session_state.inn_value:
    st.session_state.inn_value = inn
